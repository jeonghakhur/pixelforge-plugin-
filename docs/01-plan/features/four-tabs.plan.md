...
- 개발자가 필요한 형식(SVG 파일/SVG 코드/React)으로 일괄 다운로드

### 성공 기준
- [x] SVG 추출 시 불필요한 속성(width/height 하드코딩) 정리
- [x] kebab-case / PascalCase 네이밍이 일관성 있게 생성
- [x] React 컴포넌트 코드가 `width`, `height`, `color` props 지원
- [x] 일괄 다운로드 3종: SVG 파일(ZIP), SVG 코드(TS), React(ZIP)
- [x] 모달 상세 패널 — SVG/React/CSS 코드 탭 미리보기 + 복사
- [x] 플러그인 재시작 후 마지막 추출 데이터 복원 (clientStorage)
- [x] 프레임/그룹 선택 시 하위 아이콘 전체 추출 지원

### 구현 완료 (현재 상태)
- **code.ts**: `exportIcons()`, `collectExportTargets()` — 재귀 순회로 하위 노드 모두 수집
- **ui.js**: 이벤트 위임, 모달 패널, `buildReactComponent()`, `buildReactFile()`, `downloadSvgZip()`, `downloadSvgCodeZip()`, `downloadReactZip()`
- **React props**: `width`, `height` 중 하나만 입력하면 나머지 동일 값 적용. `color` prop 기본값 `currentColor`
- **CSS 포함**: React 미리보기 코드 및 다운로드 파일 모두 컴포넌트 내 `<style>{css}</style>` 포함
- **SVG 코드 다운로드**: `icons-N.ts` 단일 파일 — SVG 문자열 상수 export + `icons` map + `IconName` 타입

### 추가 개선 가능
1. **[P2]** SVG SVGO 최적화 (path 단순화)
2. **[P3]** 아이콘별 개별 다운로드 버튼 (모달 내)
3. **[P3]** 검색 결과 필터 상태에서 선택 항목만 다운로드
